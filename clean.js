...
const page11Start = lines.findIndex(l => l.includes('PAGE 11:'));
const page16Start = lines.findIndex(l => l.includes('PAGE 16:'));
const penutupStart = lines.findIndex(l => l.includes('PAGE 23: PENUTUP'));

const oldContentText = lines.slice(page11Start, page16Start).join('\n');

function extractBetweenTokens(str, start, end) {
    let s = str.indexOf(start);
    if (s === -1) return '';
    let e = str.indexOf(end, s + start.length);
    if (e === -1) return str.substring(s);
    return str.substring(s, e);
}

const profilLayanan = extractBetweenTokens(oldContentText, '<!-- Col 1: Profil Layanan IT -->', '<!-- Col 2: Kondisi Ekosistem Digital -->').trim();
const ekosistemDigital = extractBetweenTokens(oldContentText, '<!-- Col 2: Kondisi Ekosistem Digital -->', '</div>\r').trim();

// P12 Sales & Marketing starts right after `C. Penempatan Strategi` h4 
// wait, we can just extract from `<!-- PAGE 12: BAB III - LAYANAN & EKOSISTEM (Part 2) -->`.
const p12ContentStart = oldContentText.indexOf('C. Penempatan Strategi');
const p12ContentStr = oldContentText.substring(oldContentText.lastIndexOf('<h4', p12ContentStart));
const salesMarketing = extractBetweenTokens(p12ContentStr, '<h4', '</div>\r\n            </div>\r\n        </app-page>').trim();

const aspekPasarOpts = extractBetweenTokens(oldContentText, '<!-- A. Aspek Pasar dan Pemilihan Solusi -->', '<!-- B. Infrastruktur Cloud -->').trim();
const cloudHardware = extractBetweenTokens(oldContentText, '<!-- B. Infrastruktur Cloud -->', '<!-- C. Hukum -->').trim();
// For Hukum, the end is exactly before the `</div>\n </div>\n </app-page>`
const hukumStr = oldContentText.substring(oldContentText.indexOf('<!-- C. Hukum -->'));
const hukum = extractBetweenTokens(hukumStr, '<!-- C. Hukum -->', '</div>\r\n\r\n            </div>\r\n        </app-page>').trim();

const aspekSosial = extractBetweenTokens(oldContentText, '<!-- D. Aspek Sosial -->', '<!-- E. Aspek Teknis (SOP Dev) -->').trim();
const sopDevStr = oldContentText.substring(oldContentText.indexOf('<!-- E. Aspek Teknis (SOP Dev) -->'));
const sopDev = extractBetweenTokens(sopDevStr, '<!-- E. Aspek Teknis (SOP Dev) -->', '</div>\r\n\r\n            </div>\r\n        </app-page>').trim();

const tenagaKerjaStr = oldContentText.substring(oldContentText.indexOf('<!-- F. Tenaga Kerja -->'));
const tenagaKerja = extractBetweenTokens(tenagaKerjaStr, '<!-- F. Tenaga Kerja -->', '</div>\r\n\r\n            </div>\r\n        </app-page>').trim();
...
function aspectsFix(str, oldHeader, newHeader) { return str.replace(oldHeader, newHeader); }

// Keuangan Block
let keuanganText = lines.slice(page16Start, penutupStart).join('\n');
...
console.log('Restructure clean and perfect.');
